...
@objc public class NICardManagementAPI: NSObject {
    ...
    @objc public static func displayCardDetailsForm(input: NIInput, viewController: UIViewController, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
    ...
    @objc public static func setPinForm(input: NIInput, viewController: UIViewController, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        setupDisplayAttributes(input.displayAttributes)
        setPinForm(input: input, type: .dynamic, viewController: viewController, completion: completion)
    }
    
    @objc public static func setPinForm(input: NIInput, type: NIPinFormType, viewController: UIViewController, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        if viewController is UINavigationController {
            completion(nil, NIErrorResponse(error: NISDKErrors.NAV_ERROR))
        }
        setupDisplayAttributes(input.displayAttributes)
    ...
    @objc public static func changePinForm(input: NIInput, viewController: UIViewController, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        setupDisplayAttributes(input.displayAttributes)
        changePinForm(input: input, type: .dynamic, viewController: viewController, completion: completion)
    }
    
    @objc public static func changePinForm(input: NIInput, type: NIPinFormType, viewController: UIViewController, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        if viewController is UINavigationController {
            completion(nil, NIErrorResponse(error: NISDKErrors.NAV_ERROR))
        }
        setupDisplayAttributes(input.displayAttributes)
    ...
    @objc public static func getCardDetails(input: NIInput, completion: @escaping (NICardDetailsResponse?, NIErrorResponse?) -> Void) {
        retrieveCardDetails(input: input, completion: completion)
    }
    
    @objc public static func setPin(pin: String, input: NIInput, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        shared.setPin(pin, input: input, completion: completion)
    }
    
    @objc public static func changePin(oldPin: String, newPin: String, input: NIInput, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        shared.changePin(oldPin: oldPin, newPin: newPin, input: input, completion: completion)
    }
    
    
    // MARK: - Private
    private static func retrieveCardDetails(input: NIInput, completion: @escaping (NICardDetailsResponse?, NIErrorResponse?) -> Void) {
    ...
    private func setPin(_ pin: String, input: NIInput, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        PinManager().setPin(pin, input: input) { response, error in
            if let error = error {
                completion(nil, error)
            } else if response != nil {
                let result = NISuccessResponse(message: "Pin set successful!")
                completion(result, nil)
            }
        }
    }
    
    private func changePin(oldPin: String, newPin: String, input: NIInput, completion: @escaping (NISuccessResponse?, NIErrorResponse?) -> Void) {
        PinManager().changePin(oldPin: oldPin, newPin: newPin, input: input) { response, error in
            if let error = error {
                completion(nil, error)
            } else if response != nil {
                let result = NISuccessResponse(message: "Pin changed successfuly!")
                completion(result, nil)
            }
        }
    }
