...
///
public enum NICryptoStatus : CCCryptorStatus, CustomStringConvertible, Error
{
    /// Successful
    case success,
    /// Parameter Error
    ...
    {
        return (NICryptoStatus.descriptions[self] != nil) ? NICryptoStatus.descriptions[self]! : ""
    }
    ///
    /// Create enum value from raw `CCCryptorStatus` value.
    ///
    public static func fromRaw(status: CCCryptorStatus) -> NICryptoStatus?
    {
